...
unsigned int NukeOGL::getRenderTexture() {
	return fbo_texture;
}

void NukeOGL::setOnClose(b::function<void()> cb) {
	_onClose.push_back(cb);
}

void NukeOGL::setOnGUI(b::function<void(void)> cb) {
	_onGUI.push_back(cb);
	//        cout << cb << " -- onGUI[" << this << "]" << endl;
}

void NukeOGL::setOnRender(b::function<void(void)> cb) {
...
void NukeOGL::passmove(int x, int y)
{
	//cout << "pmov [ " << x << ", " << y << " ]" << endl;
...
void NukeOGL::keyboard(unsigned char c, int x, int y) {
	KeyBoard::getSingleton()->key(c, x, y);
	if (_UIkeyboard)
		_UIkeyboard(c, x, y);
}

void NukeOGL::keyboardUp(unsigned char c, int x, int y) {
	KeyBoard::getSingleton()->keyup(c, x, y);
	if (_UIkeyaboardUp)
		_UIkeyaboardUp(c, x, y);
}

void NukeOGL::special(int key, int x, int y) {
	KeyBoard::getSingleton()->special(key, x, y);
	if (_UIspecial)
		_UIspecial(key, x, y);
}

void NukeOGL::specialup(int key, int x, int y) {
...
void NukeOGL::mousescroll(int button, int dir, int x, int y) {
...
void NukeOGL::reshape(int w, int h) {
	if (_UIreshape)
		_UIreshape(w, h);
...
void NukeOGL::initFrameBufferDepthBuffer(void) {
...
void NukeOGL::initFrameBufferTexture(void) {
...
void NukeOGL::initFrameBuffer(void) {
...
int NukeOGL::init(int w, int h)
{
	...
	char* myargv[1];
	int myargc = 1;
	myargv[0] = (char*)("NukeEngine Editor");
...
void NukeOGL::DrawGrid(float HALF_GRID_SIZE)
{
	float floor = 0.0;
...
void NukeOGL::renderObject(Mesh* mesh, Material* mat, Transform* transform) {
...
char* NukeOGL::getEngine()
{
	return (char*)"Open GL 2.1 - glut";
}

char* NukeOGL::getVersion()
{
	return (char*)"1.0.0";
}
